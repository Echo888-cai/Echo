...
- 讨论同业/竞对的估值倍数（PE/EV-Sales 等具体数字）时，只能引用上面"同业对照"里列出的公司和倍数；讨论下一次财报/业绩日时，只能引用上面"下一业绩日"给的日期；两者都没核到时只能说"未核到"，不能凭自己的知识编造其它公司、倍数或日期。
...
function compPeersPromptLine(compPeers) {
  if (!compPeers || compPeers.providerStatus !== "ok" || !compPeers.peers?.length) {
    return "；同业数据未核到（Finnhub 无法识别港股 ADR 映射，或同业清单为空），讨论同业时只能说明未核到，不能凭自己的知识编造具体公司或倍数";
  }
  ...
  const anchor = compPeers.anchor
    ...
    : "；同业数量不足未生成锚点";
  return `\n  同业对照（Finnhub 自动匹配，讨论同业/竞对估值倍数时只能引用这里列出的公司和倍数，不能凭自己的知识编造其它公司或倍数）：${peerList}${anchor}`;
}
