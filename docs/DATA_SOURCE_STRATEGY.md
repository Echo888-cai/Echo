...
| 数据层 | 当前实现 | 状态 |
|--------|----------|------|
| 市场识别 | `src/market.js`：HK/US 自动识别 + 各源 symbol 映射 + 币种 | 已接入 |
| 公司 universe | `src/data/hkStocks.js` + SQLite `companies`（港股）；美股按 ticker 即时建档 | 已接入 |
| 行情 | 港股 Tencent；美股 Finnhub/Alpha Vantage/Yahoo；`market_snapshots` 缓存 | 已接入 |
| 财报三表 | **美股 FMP `/stable` 真财报**；港股回退腾讯/Yahoo 基础 | 美股已通 / 港股受限 |
| 财务质量评分 | `financialQuality.js`（美股真分，如 AAPL 83/100） | 已接入 |
| 估值与赔率 | `valuationEngine.displayValuation`（自洽 PE 带 + 赔率） | 已接入 |
