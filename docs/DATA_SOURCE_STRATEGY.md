...
- **行情**：腾讯（`qt.gtimg.cn`，`sh`/`sz` 前缀）+ 新浪（`hq.sinajs.cn`，需 `Referer` 头）双源免 key，实测均可用；配置 `EODHD_API_KEY` 后，在两者失败时回退到 EODHD 的**日终收盘价**（A股代码自动映射为 `.SHG` / `.SHE`），不会把收盘数据标成实时。FMP/Finnhub/Alpha Vantage 免费档均不覆盖 A股，只作为理论兜底（几乎不会命中）。
