...
- [x] 财报日历（美股部分）：`packages/data-plane/src/adapters/finnhubCalendarAdapter.ts` 接 Finnhub `/calendar/earnings`（真实探测确认端点不带 `from`/`to` 参数会静默返回空数组而非报错，端点本身只覆盖美股，HK 返回和行情/财务同款"无权限"），注册进 `registry.ts` 的 `calendarAdapters`（`postgresCalendarAdapter` 降级为 rank 2 兜底，因为它读的缓存表当前无任何写入方）。真实回测：AAPL 命中 finnhub 返回 2026-07-30 财报日与预期 EPS/营收；0700.HK/600519.SS 落到 postgres 缓存——过程中发现该缓存表里已有几行"经 ADR 映射查到"的港股数据，但 `upsertEarningsCalendar` 在全仓库没有任何调用点，说明这是此前某次未提交的临时脚本写入的冻结脏数据，已用 spawn_task 记录为独立待办（是否值得实现港股 ADR 映射，或应先清理冻结数据）。同业（comp_peers）尚未接入，同样只在装饰性 env-key 判断里。
