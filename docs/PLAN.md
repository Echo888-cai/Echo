...
| C1–C2 | 移动端响应式 + PWA | 🟡 2026-07-05 | **顺手修复**（D1 收尾后，非本次主线，用户明确"看情况/顺手"授权）：真机宽度（375px）实测发现整页级横向溢出 bug——`.workspace` 在 ≤1080px 收成单列网格后，`.sidebar` 网格项默认 `min-width: auto` 撑开轨道宽度，导致导航栏被切、通知徽标溢出、决策面板文字被裁切（`scrollWidth` 467 vs `clientWidth` 375）。给 `.sidebar`/`.desk` 补 `min-width: 0` 一行修复；浏览器实测 375px/768px/1440px 三档、研究页/看盘列表/看盘个股详情/设置页四个页面，无回归。**范围裁剪**：这只是修复了阻断性的结构 bug，不是完整的 C1 响应式打磨（触控区加大、侧栏抽屉化等仍是 ⬜）；C2（PWA manifest + service worker）未动。 |
...
| D1 | `src/server/**`/`src/ui/**` 渐进 JSDoc + `checkJs` | ✅ 2026-07-05 | **第一批**（2026-07-04）：范围收窄到 dataSources/valuation/research session/watchlist/portfolio/chat orchestration 六类跨模块对象，基建落地 `jsconfig.json`（`allowJs`+`checkJs`，non-strict，`maxNodeModuleJsDepth:0` 滤掉 node_modules 噪音）+ `typescript`/`@types/node` devDependency + `npm run typecheck`（不接入 CI，纯本地提示）+ `src/server/types.js` 集中 `@typedef`。**第二批**（2026-07-05，收尾）：把 `npm run typecheck` 从"六个目标文件 0 error"推到"项目全量 0 error"——逐一修完 `src/server/**` 剩余全部文件（`discovery.js`/`agentTools.js`/`webEvidenceService.js`/`intentClassifier.js`/`companyPortrait.js`/`eventEngine.js`/`riskEngine.js`/`financialQuality.js`/`hkFilingsPipeline.js`/`notifier.js`/`agentService.js`/`data.js`/路由层等）与 `src/app.js`+`src/ui/**` 全部 13 个文件。**意外收获（最重要的一条）**：`checkJs` 在 `valuationEngine.js` 的 `computeValuation()` 里揪出一个真实的估值计算 bug——`hasPrice`（`price !== null && price !== undefined` 算出的布尔值）在 PB 估值（金融股）、DCF 缺股本兜底（`marketCap/price` 反推股数）、简单 PE 兜底（`price/pe` 反推 EPS）三处被误写成除数，JS 里 `true` 强转成 `1`，导致这三条估值路径实际全在拿"1"当现价用，算出的 bear/base/bull 和真实现价完全脱节；本地脚本复现验证：修复前 PB 法对 price=100/pb=1.5x 会算出 base≈1.00，修复后正确算出 base=100.00。**其他副产品**：`checkJs` 顺手抓到 4 处真实 JSDoc 语法错误（`scheduler.js`/`watchDesk.js`/`eventEngine.js` 的 `@param`/`@returns` 漏花括号，导致 TS 解析中断、连累后面报出无关错误）+ 若干处 doc-drift（JSDoc 参数名和实际形参不一致，如 `riskEngine.js`/`financialQuality.js`）+ 两处 `[RegExp, entry]` 数组因缺显式 tuple 类型标注被 TS 错误展开成同构联合类型（`SECTOR_MAP`/`STATEMENT_FIELDS`）。`types.js` 的 `FinancialsData`/`Valuation` 等类型随每个文件的真实用法逐步补全字段（不是拍脑袋编类型，是被 `tsc` 报错倒逼贴合实际）。`npm run typecheck`：项目全量 0 error；`npm test` 全绿（263 用例，纯类型标注+一处 bug 修复，无行为回归）；lint 0 error。浏览器实测：`/api/chat`（腾讯估值问答，估值区间与现价自洽）、`/api/watch/desk`、首页公司速览卡点击→完整 `/api/ask` 研究链路、scheduler 启动均正常，控制台无报错。 |
| P8 | 商业化底座 | ⬜ | 需用户逐项对齐 |
