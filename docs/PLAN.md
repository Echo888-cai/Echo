...
| B-6~B-7 | filings 覆盖/web 证据非沙箱实测 | ⬜ | B 研究质量深化主线，B-5 之后推进；B-6 的优先切入点见上方"范围裁剪"（部分港股 PDF 格式解析失败） |
