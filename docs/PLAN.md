...
- ~~网页证据层~~（P3 已接通 Tavily 适配器；密钥未配时诚实降级）。
- ~~earnings_calendar 无写入方~~（2026-07-15 已接通）。
- ~~factGuard `full` 模式~~（P3 已实现拦截+定向重答闭环）。
- ~~两类通知不存在~~（P4 已接通：`position_alert` / `review_reminder` worker + 调度）。
- **历史估值分位**：美股可做（FMP 5 年年度 EPS + Yahoo 月度价格）；港股需先把 filing 回补到 ≥5 个财年。
- **应收/存货字段**：需扩数据管道（FCF 公司自报解析已接通）。
- **港股 ADR 溢价**：需逐条人工核实 9 家 ADR 比例后才能接 `dualQuote`（见第 6 节）。
- **AnswerCard 字段**：`analyst / dualQuote` 仍待上述数据源；`evidence / grounding / completeness` 与数字级可追溯已接通。
...
- ~~证伪"温度计"~~：StockDetail 可视化条 + 距阈值 / 触发历史 / 行情新鲜度。
- ~~业绩期驾驶舱~~：StockDetail「业绩」Tab，预期/实际/surprise 同屏；披露后 30 分钟实时触发仍待。
- ~~`position_alert` / `review_reminder`~~：worker activity + Temporal 调度已接通；设置页开关生效。
- ~~研究记忆 schema~~：`research_facts` / `research_questions` / `review_dates` + repository（自动提取接线仍待）。
- ~~静默时段~~：`quiet_hours_*` 偏好 + `insertNotification` 咽喉拦截（证伪/持仓紧急类绕过）。
- ~~新手引导打磨~~：示例公司入口 + 动效；空态/失败恢复仍可继续打磨。

**仍待：**

- 研究记忆自动提取进研究管线；日/周摘要只推有新证据/近证伪/即将披露的变化。
- 来源偏好可调；反馈闭环（标记数字错误/来源失效）进可追踪队列 UI。
- 业绩披露后 30 分钟内出第一版复盘（需更高频轮询或 webhook）。
- 管理层口径变化跟踪。
...
- ~~字段级来源登记表~~：`data_source_fields` schema + repository。
- ~~团队空间 / 审计日志 / 套餐 schema~~：`teams` / `team_memberships` / `audit_log` / `plans` / `subscriptions` + repository。
- ~~服务端 Markdown 导出~~：证据链 + 合规水印 + 观点时间线（`exports.research`）。
- ~~负载测试场景~~：`npm run test:load:scenarios`（真实用户旅程加权）。

**仍待（需外部合同 / 基础设施）：**

- **第一优先：港股商用行情源替换**。港股行情目前唯一真实覆盖是 Yahoo chart 接口，`commercialUseAllowed=false`——商业化前必须换成有授权的源（港交所 OMD 转售商、EODHD、iTick 等）。
- 托管 PostgreSQL、Temporal Cloud、对象存储、OTel 与告警在预生产完整接通；蓝绿切换、恢复、故障与降级联合演练。
- `npm run test:load` 预生产实测并发研究、长报告、披露高峰与供应商限流。
- 团队空间前端、细粒度权限 UI、共享模板、评论；PDF 导出。
- 支付集成、账单管理；密钥轮换与最小权限审计；渗透测试、隐私政策、数据处理协议与法务发布审查。
