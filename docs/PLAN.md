...
| 6 | `crates/echo-application` | `packages/application/research.ts` | 🚧 编排+估值+意图已接；DB 行→领域映射(`from_db`)+4 项测试绿；模型网关**非流式核心已接**(`model_gateway`：provider 选择 DeepSeek→OpenAI→通用 / OpenAI 兼容请求体 / 作答提取 / `parse_json_object`，11 项纯函数测试绿)。剩余显式 seam：流式 SSE 增量、`llm_audit` 落库、活库端到端验证 |
