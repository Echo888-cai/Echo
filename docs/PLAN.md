...
> **状态**：§2/§3 是审计当时的建议路线，现已按此路线走完；§5 状态表的 F 轨（F-1…F-5）与顺手项 P7（研究历史全文检索）全部完成，仅剩明确缓做项（P3/P4 级）待用户提出新方向。
...
| P7 | 研究历史全文检索：`013_research_sessions_fts.sql` 新建 `research_sessions_fts`（FTS5 external content 虚拟表，索引 `research_sessions` 的 title/question/report_markdown/thread_json）+ 3 个触发器（`AFTER INSERT/UPDATE/DELETE`）自动同步索引——`researchSessions.js` 里现有的 upsert（`INSERT ... ON CONFLICT DO UPDATE`）/delete 代码**完全不用改动**，触发器在 DB 层接管。`tokenize='trigram'`：中文没有空格分词，unicode61 默认分词器切不出中文词；trigram（3 字符滑动窗口子串索引）不依赖分词就能支持中文子串搜索，真实验证 better-sqlite3 3.53.2 内置支持。**已知限制**（trigram 索引固有特性，非 bug）：查询串短于 3 个字符匹配不到任何结果——`searchResearchSessions`/路由层/前端三层都诚实处理（分别返回空数组/`tooShort:true`/"再多打几个字"提示），不让用户把"太短搜不到"误解成"真没有"。**安全设计**：`snippet()` 高亮定界符用两个不可打印控制字符（U+0001/U+0002）当占位符，不直接用字面 `<b>`/`</b>`——命中片段摘自用户问题/模型正文，可能夹带 `<`/`>` 字符，前端 `highlightSnippet()` 先对整段转义（`esc()`）、再把占位符替换成真正的 `<b>`/`</b>`，避免命中内容里的原始字符被当成标签注入页面（存储型 XSS）。前端：侧栏历史面板新增搜索框（`sidebar.js`），300ms 防抖（`app.js`），查询串非空时整个区域切换为扁平搜索结果（不与原有分组列表同屏，避免"两套列表"造成混淆）；`render()` 里新增对 `.history-search-input` 的焦点/光标保留逻辑，与既有的 composer textarea 保留同一个模式，防止防抖触发的重渲染打断正在输入。真实数据验证：真实浏览器对已有 27 条真实历史研究搜索"估值区间"，20 条真实匹配全部正确返回（含真实高亮片段，如"…理，二是在**估值区间**框架下赔率…"），点击搜索结果正确恢复完整历史研究会话（腾讯控股 0700.HK 面板+对话）；1 字符查询正确显示"再多打几个字"提示，未发出网络请求。`tests/phase-p7.mjs`（8 项：触发器自动同步增删改、查询串过短诚实降级、snippet 占位符不含字面标签、重复 upsert 不产生重复索引行），`tests/phase-d2.mjs` 迁移版本断言更新到 13（新增 `research_sessions_fts` 虚拟表存在性检查）。全套测试/lint/typecheck 干净。 | 2026-07-06 |
