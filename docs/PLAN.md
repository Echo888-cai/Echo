...
端与端之间只有 zod 契约；多步任务由 Temporal 提供可重放语义（本地不可达时降级为内联执行并诚实标注）；结构化研究数据保留 valid time 与 knowledge time。仓库布局见 [architecture/repository-layout.md](architecture/repository-layout.md)，部署与 CI 见 [architecture/system-overview.md](architecture/system-overview.md)。
...
- **研究链路**：`answerComposer`（意图分类路由段落结构）+ `reportComposer`（深度报告与对话回答共享取数、分开渲染）+ 估值区间（bear/base/bull）+ 同业可比（`comp_peers`，24h TTL，倍数可比上限过滤离群值）。
...
- **真流式**：`/api/ask` SSE 真实转发 provider delta（按 ~24 字符合并），首 token < 3s；管线阶段事件驱动等待提示。
- **门禁**：冻结表 CI 门禁 `check:frozen`（写入方零调用=表永远空；读取方零调用=数据白采，双向拦截）；退役栈门禁 `check:retired`；契约输出 schema；删除闭环 E2E；canary 真探测驱动状态页。
...
| # | 事项 | 依赖 |
| --- | --- | --- |
| IX-1 | **流式体验打磨**：阶段事件全覆盖（取数/核对/成文）、骨架屏、失败恢复与断线续读 | 无 |
...
| IX-6 | **新手引导完成率**：示例公司入口 → 首个研究 → 首个证伪条件的漏斗打点与打磨 | 无 |
| IX-7 | **导出体验**：Markdown 导出（已有）→ 带证据链与水印的 PDF | 无 |
