...
| D2 | 轻量 DB 迁移器（`user_version` + `migrations/NNN_*.sql`） | ⬜ | 最高风险优先：11 处 `ensureTable()` 收敛成集中迁移文件，`PRAGMA user_version` 追踪已应用版本，现有 14 张表的 `CREATE TABLE` 定义逐字迁移（不改字段/约束，零 schema 变更），迁移前对真实 `luvio.db` 做文件级备份；回归测试覆盖"全新库跑到最新版本"+"现有旧库无损迁移"两种场景 |
| D3 | `chat.js` 编排逻辑内化进工具层 | ⬜ | `runChat`/`finalizeChat`/`buildFinalThread` 等 ~400 行搬进 `src/server/services/`（具体文件待定，可能新增 `chatOrchestrator.js`），供 `agentTools.js` 的 `researchCompany`/`compareCompanies` 直接复用而非薄封装外层；沿用 EA-1 做法，改动前后输出逐字节快照对比，`discover.js` 已足够薄，不在本阶段重复处理 |
| D1 | `src/server/**`/`src/ui/**` 渐进 JSDoc + `checkJs` | ⬜ | 放在 D2/D3 定型之后处理，避免给即将搬迁的代码补两遍类型注释；~1.4 万行分批覆盖，先加 `jsconfig.json` 开 `checkJs`（仅提示不阻断构建），再从高频改动的核心服务（`agentTools.js`/`chat.js` 新编排层/`db/`）往外扩 |
| P8 | 商业化底座 | ⬜ | 需用户逐项对齐 |
