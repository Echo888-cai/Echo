...
| Ingest HK financials | `POST /api/hk-financials/ingest` | `cargo xtask hk-discover <ticker>` → HKEX 官方公告清单；`cargo xtask hk-ingest <json>` → `normalize_hk_financials` → `HkFinancialsRepository::upsert_normalized` | skeleton | HKEX 当前 JSONP/搜索响应 fixture + 期间解析 + 0700.HK 真实端点；单位别名/单次换算/EPS 不换算/HKEX 来源/负营收/迁移注册单测 | keep | Phase 2–3 | 已完成自动公告发现、期间解析、安全结构化写入与单位证明；公共 HTTP 写入口刻意不开放。PDF 文本抽取与三表行解析仍待迁移，故发现结果暂不自动写库。 |
...
| 网页证据检索（Exa/Tavily） | `tavilySearchAdapter` | `echo-data::EvidenceService`（**双供应商**：`EXA_API_KEY` 优先走 Exa `/search`，否则回落 Tavily `/search`）+ `web_evidence` 24h 缓存 + `ResearchPorts::load_web_evidence` + `answer_prompt` 证据块 + `AskResponse.sources` + Web `SourceCards` | rust-accepted | echo-data 单测（供应商优先级/query 拼接/域名解析/截断/双响应形状/缓存键供应商隔离/日期往返/商用+缺 key 降级）+ application 单测（意图门控、对比两腿分别取证）+ answer_prompt 单测（单主体 `[N]`、对比 `[A1]/[B1]`）+ **live 浏览器端到端**（2026-07-24 真 Exa：AAPL 护城河问题→5 条来源→答案引用→Web 来源卡；反向估值意图 0 源） | keep | P2-1 | 有库按 ticker/provider/query 读 24h 缓存，未命中回源并原子替换；供应商失败或空响应可用陈旧缓存降级，无库保持实时。仅支撑定性，绝不进 `FactsRegistry`；商用模式继续拒绝未授权免费源。对比两腿按 `[A1]/[B1]` 隔离。剩余增强：港股中文源专用适配器。 |
...
| 2026-07-24 | 对比研究两腿证据闭环：此前 `assemble_facts` 已分别检索 A/B 两腿，但 `CompareLegContext`/`CompareLegView` 没消费，属于取数后冻结。现按 `[A1]`/`[B1]` 命名空间把各腿证据分别送入对比 prompt，并随各自 `CompareLegView.sources` 回到 Web 双栏；提示词明确禁止 A/B 来源串腿，单测锁定两腿各调用一次证据端口。 |
| 2026-07-24 | 网页证据缓存：复用既有 `web_evidence` 表，新增 `WebEvidenceRepository`，按 ticker/provider/query 读取 24h 缓存，回源成功原子替换，供应商失败或空响应时允许陈旧缓存降级；无 `DATABASE_URL` 保持实时路径。缓存行 ID 用 SHA-256 覆盖供应商/检索式/URL，避免 Exa/Tavily 与不同问题串缓存。 |
| 2026-07-24 | 流式阶段与计划对齐：`ResearchStreamStage` 增加 1-based `index/total`，服务端按 `route.plan` 依次发 `routing/resolving/market_financials/evidence/valuation/generating/fact_check`；核心事实与网页证据拆成独立 IO 边界。Web 显示“第 N/M 步”与进度条。修正旧计划与执行不一致：brief 只缩短答案/检索广度，不再悄悄从 plan 删除实际仍会执行的 evidence。 |
| 2026-07-24 | HKEX 公告发现迁移：`HkexService` 通过披露易 `prefix.do` 精确解析 stockId，再查 `titleSearchServlet.do`；过滤更正/补充等噪音，只保留官方 PDF，按香港本地发布时间转 UTC，并从常见中文标题解析 FY/Q1–Q4/H1/9M。`cargo xtask hk-discover 0700.HK` 已在当前真实端点验证，返回 2026 Q1、FY2025 等 10 条公告。PDF 文本/三表解析仍是下一层。 |
