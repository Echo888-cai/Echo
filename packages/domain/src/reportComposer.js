...
export function createReportComposer({ researchStatusLabels, companyByTicker, beijingMinute }) {
...
function reportPreview(panel) {
  if (!panel) return "";
  return String(panel.oneLineView || panel.action || "").slice(0, 200);
}

return { composeReport, reportPreview };
}
