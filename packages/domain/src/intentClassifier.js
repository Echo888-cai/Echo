 ...
 * 证伪各有专属段落结构，而不是所有问题都套同一份完整研究模板）；这正是
 * docs/PLAN.md P2"意图分类驱动提示词"的那一半。规则原样取自旧底盘
 * （ce58d27:src/server/services/intentClassifier.js），架构换血时随旧栈一起删掉，
 * 这里按新分层重新落位：分类是领域规则，注入 composer 的时钟/档案端口留在应用层。
 *
