...
console.log("[4.5] 回归：金额桶跨数量级时按对数距离找最近事实，不按绝对差");
{
  ...
  // 真实影子模式抓到：AAPL 回答里"对应股价约 220-250 美元"，区间写法的连字符被当成负号，
  // 抓出"-250"跟正数的"估值中性"事实符号相反，误判 hard。
  const rangeText = "对应股价约 220-250 美元区间。";
  const vRange = verifyAnswerNumbers(rangeText, registry);
  check("区间写法的连字符不被当成负号", !vRange.checked.some((c) => c.value < 0), JSON.stringify(vRange.checked));
  check("区间右端正确抽成正数", vRange.checked.some((c) => c.dimension === "amount" && c.value === 250), JSON.stringify(vRange.checked));

  // 真实影子模式抓到：小米"收入同比下滑10.9%"没写负号（中文财经写作常见表达），跟登记表
  // 里真实的 -10.9 符号相反被误判 hard；识别"下滑/下降"这类降幅词，按语义当负数处理。
  const declineRegistry = buildFactsRegistry({
    ticker: "TEST",
    marketSnapshot: { providerStatus: "ok", price: 100, currency: "HKD" },
    financialsData: { providerStatus: "ok", currency: "HKD", revenueGrowth: -10.9 }
  });
  const declineText = "收入同比下滑10.9%，拖累整体表现。";
  const vDecline = verifyAnswerNumbers(declineText, declineRegistry);
  check("下滑 X% 不写负号也能按负数正确匹配", vDecline.checked.some((c) => c.dimension === "percent" && c.verdict === "pass"), JSON.stringify(vDecline.checked));
  const growthText = "收入同比增长10.9%，表现良好。"; // 反例：增长词不该被当负数
  const vGrowth = verifyAnswerNumbers(growthText, declineRegistry);
  check("增长 X% 不会被误当负数（真实是正数应该 soft/hard 而不是巧合 pass）", !vGrowth.checked.some((c) => c.dimension === "percent" && c.verdict === "pass"), JSON.stringify(vGrowth.checked));

  // 真实深度报告回测抓到：模型写"同比微降1.1%""同比仅微降0.5%"——语义完全正确，却因为
  // "微降"不在原来写死的双字词名单里被当成 +1.1% 判 hard。降/跌/滑 前面可挂任意修饰语，
  // 穷举不完，故改认词尾方向字。
  const softDeclineRegistry = buildFactsRegistry({
    ticker: "TEST",
    marketSnapshot: { providerStatus: "ok", price: 100, currency: "HKD" },
    financialsData: { providerStatus: "ok", currency: "HKD", revenueGrowth: -1.06, profitGrowth: -0.51 }
  });
  for (const [label, text] of [
    ["微降", "收入1943.71亿人民币，同比微降1.1%。"],
    ["仅微降", "净利润590.89亿人民币，同比仅微降0.5%。"],
    ["环比略降", "收入环比略降1.1%。"],
    ["大幅下滑", "收入同比大幅下滑1.1%。"]
  ]) {
    const v = verifyAnswerNumbers(text, softDeclineRegistry);
    check(`"${label} X%" 按负数匹配、不误判 hard`, !v.checked.some((c) => c.dimension === "percent" && c.verdict === "hard"), JSON.stringify(v.checked));
  }
  // 反例守住：方向字泛化后，"增长/提升/回升"仍然不能被当成负数。
  const vRise = verifyAnswerNumbers("收入同比回升1.1%。", softDeclineRegistry);
  check("回升 X% 不会因为泛化匹配被误当负数", !vRise.checked.some((c) => c.dimension === "percent" && c.value < 0), JSON.stringify(vRise.checked));

  // 真实深度报告回测抓到：模型写"增速超过 3%"这类假设性阈值（不陈述登记表里的任何事实），
  // 被拿去跟"今日涨跌幅 =-1.5%"比——2 倍差也落在旧的 1/3~3 倍 sameBallpark 窗口里，
  // 直接误判 hard。符号相反只应在绝对值基本相等时成立。
  const thresholdRegistry = buildFactsRegistry({
    ticker: "TEST",
    marketSnapshot: { providerStatus: "ok", price: 100, currency: "HKD", changePercent: -1.5 }
  });
  const vThreshold = verifyAnswerNumbers("若收入恢复正增长且增速超过3%，逻辑才重新成立。", thresholdRegistry);
  check("假设性阈值 3% 不会因为落在负数事实 3 倍内被误判 hard", !vThreshold.checked.some((c) => c.dimension === "percent" && c.verdict === "hard"), JSON.stringify(vThreshold.checked));
  check("假设性阈值 3% 判为 soft（未核到）", vThreshold.checked.some((c) => c.dimension === "percent" && c.verdict === "soft"), JSON.stringify(vThreshold.checked));
  // 真符号错误必须照抓：绝对值相等、方向写反。
  const vRealFlip = verifyAnswerNumbers("今日上涨1.5%。", thresholdRegistry);
  check("绝对值相等的真符号错误仍判 hard", vRealFlip.checked.some((c) => c.dimension === "percent" && c.verdict === "hard"), JSON.stringify(vRealFlip.checked));
}
