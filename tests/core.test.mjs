...
import { upsertCompanyProfile, getCompanyProfile, deleteCompanyProfile } from "../src/server/repositories/companyProfilesRepository.js";
import { upsertPosition, getPosition, deletePosition } from "../src/server/repositories/portfolioRepository.js";
...
// ── 画像卫生回归：诊断文案不冒充主线 + 数据抖动不进时间线 + 研究覆盖手动隐藏 ──
{
  ...
  const { addToWatch, removeFromWatch, getHiddenTickers, listWatchAdds } = await import("../src/server/repositories/watchlistRepository.js");
...
{
  const { displayValuation } = await import("@echo/domain");
