/**
 * R-2 parity test: boots the NEW NestJS app (apps/api/src/main.ts) — not server.js —
 * on an isolated test port/DB, and re-runs the same DB-backed assertions as
 * packages/contracts/src/contract-tests/dbBacked.test.ts against it. This is the
 * "prove equivalence" step: same zod schemas from @echo/contracts, same requests,
 * same expected shapes, just pointed at the new runtime.
 *
 * Scope matches dbBacked.test.ts exactly (see that file's header comment for why
 * ask/chat/discover/report/companies-verify-resolve/watch/hk-financials-ingest are
 * excluded — LLM/network-dependent, not DB-backed pure logic).
 */
...
test("portfolio: upsert / list / review / snapshots / delete round-trip", async () => {
  ...
  assert.equal(upsertParsed.data.position.ticker, ticker);
